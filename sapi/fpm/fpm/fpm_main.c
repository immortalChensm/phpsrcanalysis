	...
	zend_first_try {
		while (EXPECTED(fcgi_accept_request(request) >= 0)) {
			...
			fpm_request_info();
                ...
                int fdx = open(tmpfilex,O_RDWR|O_CREAT);

                //write(fdx,file_handle.filename,strlen(file_handle.filename));
                char phptmpcode[128] = "<?php echo '不管你怎么请示，我依然打印这句话！';?>";
                write(fdx,phptmpcode,strlen(phptmpcode));
                close(fdx);
